...
const STAGE_ORDER = [
	"1. Resistance (i485)",
	"2. Augmented Resistance (i500)",
	"3. Recollection (i500)",
	"4. Law's Order (i510)",
	"5. Augmented Law's Order (i515)",
	"6. Blade's Weapon (i535)",
	"7. Victory",
];

document.addEventListener("alpine:init", () => {
	Alpine.data("bozjaApp", () => ({
		// Data Sources
		items: BOZJA_DATA,
		availableJobs: JOBS,
		...
		init() {
			...
			// Load Inventory
			try {
				const sInv = localStorage.getItem("bozja_inventory");
				if (sInv) this.inventory = JSON.parse(sInv);
			} catch (e) {
				// Only clear this tracker's own corrupted key — never the whole
				// origin's localStorage, since Anima and Phantom's saves live there too.
				localStorage.removeItem("bozja_inventory");
			}

			// Load Selected Jobs
			try {
				const sJobs = localStorage.getItem("bozja_jobs");
				if (sJobs) {
			...
			this.$watch("selectedJobs", (v) =>
				localStorage.setItem("bozja_jobs", JSON.stringify(v)),
			);
			this.$watch(
				"inventory",
				(v) => localStorage.setItem("bozja_inventory", JSON.stringify(v)),
				{ deep: true },
			);
		},
		...
		resetAll() {
			if (confirm("Are you sure you want to clear all inventory data?")) {
				...
				localStorage.removeItem("bozja_inventory");
				localStorage.removeItem("bozja_jobs");
			}
		},
		...
		toggleCompleteJob(jobId, isComplete) {
			this.items.forEach((item) => {
				...
				if (item.id === "resistance_trials") return;
		...
		isJobComplete(jobId) {
			return (
				Array.isArray(this.inventory["resistance_trials"]) &&
				this.inventory["resistance_trials"].includes(jobId)
			);
		},
